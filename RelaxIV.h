 ...
 * D. Bertsekas. Conforms to the standard (MCF) interface defined in
 * MCFClass.h.
 *
 * The RelaxIV is based on a primal-dual algorithm which essentially operates
...
/**< Decides if the auction/shortest paths inizialization procedure is used.
   ...
   is provided [see SetAuction() below] that has been reported to make the
...
/*--------------------------------------------------------------------------*/
   ...
   inline cIndex_Set MCFSNdes( void );
...
/**< Same meaning as MCFClass::MCFSNdes().
   ...
   inline cIndex_Set MCFENdes( void );
...
/*--------------------------------------------------------------------------*/
 ...
 unsigned long iter;                // number of iterations (of both types)
 unsigned long num_augm;            // number of flow augmentation steps
 #if( RELAXIV_STATISTICS )
  unsigned long nmultinode;         // number of multinode iterations
  unsigned long num_ascnt;          // number of multinode ascent steps
  #if( AUCTION )
   unsigned long nsp;               // n. of auction/shortest path iterations
  #endif
 #endif
...
inline void RelaxIV::GetPar( int par, int &val )
{
 if( par == kAuction )
  #if( AUCTION )
   val = crash ? kYes : kNo;
