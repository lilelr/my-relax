...
/*--------------------------------------------------------------------------*/
    ...
    typedef int FNumber;        ///< type of arc flow
...
/*--------------------------------------------------------------------------*/
    ...
    typedef int CNumber;        ///< type of arc flow cost
...
/** Public enum describing the possible file formats in WriteMCF(). */
    ...
    enum MCFFlFrmt {
        kDimacs = 0,    ///< DIMACS file format for MCF
...
inline void MCFClass::CheckDSol(void) {
    ...
    for (Index i = 0; i < MCFm(); i++) {
        ...
        } else if (GTZ(tRC[i], EpsCst) && GTZ(tX[i], EpsFlw))
